...
function lfs.attributes(filepath, attribute_name_or_table)
	...
	if filesystem.isLink(filepath)
		tmp.mode = "link"
	elseif filesystem.isDirectory(filepath)
		tmp.mode = "directory"
	elseif filesystem.exists(filepath)
		tmp.mode = "file"
	else
		return nil, "ERROR: Item is not a link, directory, or a file! (Might be a bug)"
	end
	tmp.dev = -1
	tmp.ino = -1
	tmp.nlink = -1
...
function lfs.symlinkattributes(filepath, attribute_name_or_table)
	local is_link, tmp = filesystem.isLink(filepath)
	
	if is_link then
		return lfs.attributes(tmp, attribute_name_or_table)
	else
		return lfs.attributes(filepath, attribute_name_or_table)
	end
end
